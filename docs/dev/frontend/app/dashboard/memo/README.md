...
|---------|-----|-------------|
| KPI cards | Week totals: visits (trend vs same weekday last week, not last week's full total); FD/SS scholars at 80% or more (Sophomores then Freshmen) with Progress tracks; tutoring sessions logged vs no-shows. Same meaning as the print Program Snapshot hours/tutoring tiles, dashboard Card chrome | Campus week |
...
| Recognition board | **Census** — every assignment grade parsed from this week's **WAHF** (90–100% / 70–89% / below 70%) | Latest WAHF per submitter that week, including team leaders; empty band = None |
...
Export PDF (`weekly-memo-report.service` + `weekly-memo-pdf.service`) is a staff-meeting brief, not a restyle of this page. The button fetches same-origin `GET /api/memo/pdf` (Next route `app/api/memo/pdf/route.ts`), which proxies Express with the server `BACKEND_URL` cookie JWT — it does not call Express from the browser via `NEXT_PUBLIC_BACKEND_URL`. Snapshot tiles include print-only SVG bars (`weekly-memo-pdf-charts.ts`: stacked on-time / late / missing, cohort completion tracks). Front-desk and study-session snapshot rows are labeled **Sophomores** then **Freshmen** (class year from `FALL_SEMESTER_FIRST_DAY`), not entering-year numbers. Those hours tiles count a scholar complete at **80% or more** (logged minutes plus `scholar_week_excuses`); the page KPI hours cards use the same counts. That is not the page pie’s 100% bar. The snapshot groups scholar tiles (hours, tutoring, scholar WAHF) separately from team-leader tiles (TL WAHF, WPL, MCF). Room traffic (week trend + Mon–Fri heat map from `trafficSessions`) is its own page. Needs Attention lists scholar WAHF names separately; TL WAHF, WPL, and MCF share Team Leader Submissions. Study-session and front-desk appendix (and Needs Attention hours) use the same completion math as Full attendance detail: logged minutes plus `scholar_week_excuses`, rounded to an integer percent and capped at 100. Those rosters are grouped by class year (Freshmen, Sophomores) and sorted by completed minutes descending. Snapshot, Room Traffic, Needs Attention, and each appendix start on their own page. The download filename and print masthead/footer include an Eastern printed-at timestamp.
...
| `weekly-memo-export-button.tsx` | [source](https://github.com/College-Success-Scholars/css-atlas-v2/blob/develop/frontend/app/dashboard/memo/_components/weekly-memo-export-button.tsx) | Export PDF — same-origin `/api/memo/pdf` download |
| `weekly-kpi-cards.tsx` | [source](https://github.com/College-Success-Scholars/css-atlas-v2/blob/develop/frontend/app/dashboard/memo/_components/weekly-kpi-cards.tsx) | Weekly KPI summary cards; hours cards use Progress tracks (`bg-front-desk` orange, `bg-study` blue) |
...
- **Scholar forms vs TL forms** — do not show WPL/MCF as required on scholar rows. WAHF exceptions are follow-up Issues (submitted-at when a log exists); WAHF census is on attendance detail. Assignment grades from every WAHF this week (including team leaders) are the Recognition board census; only scholar low grades are follow-up Issues. Team leader WAHF compliance stays on team leader performance. The export PDF still lists scholar WAHF separately from follow-up.
...
- **Export PDF is same-origin** — `WeeklyMemoExportButton` fetches `/api/memo/pdf`. Do not use `lib/client/api-client` / `NEXT_PUBLIC_BACKEND_URL` for the printout.
