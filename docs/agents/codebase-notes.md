...
- `attendance-week.*`: campus-week boards, mentee UID slices, and Memo minutes from cleaned tickets on read + excuses in `scholar_week_excuses` keyed by campus-week `week_start`. Frozen `*_records_legacy` tables are not used at runtime.
