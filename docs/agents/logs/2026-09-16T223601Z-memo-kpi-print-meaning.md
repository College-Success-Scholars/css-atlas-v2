# memo-kpi-print-meaning

**Date:** 2026-09-16T223601Z
...
Align weekly memo page KPI cards with print snapshot hours and tutoring meaning.
...
Traced page KPI cards to assembleWeeklyMemo, then rebuilt Front desk, Study session, and Tutoring KPIs to match print snapshot meaning while keeping Visits this week and dashboard Card chrome. Hours cards now count scholars at 80% or more (logged plus excuse minutes), with Sophomores then Freshmen subStats; tutoring uses sessions logged versus no-shows. Updated KPI titles, memo README, memo-legacy mock data, and assembler/async-content tests. Memo tests passed. Full frontend test and production build still fail on pre-existing coordinator/auth/roster working-tree changes, not these KPI files.
